...
class ADDA():
    ...
    def define_target_encoder(self, weights=None):
        ...
        with tf.device('/gpu:1'):###################'/cpu:0'-->'/gpu:1'##################
    ...
    def train_source_model(self, model, epochs=2000, batch_size=128, save_interval=1, start_epoch=0):
        ...
        saver = keras.callbacks.ModelCheckpoint('datamodify/svhn_encoder_{epoch:02d}-{val_loss:.2f}.hdf5', 
                                        monitor='val_loss', 
                                        verbose=1, 
        ...
        if not os.path.isdir('datamodify/tensorboard'):
            os.mkdir('datamodify/tensorboard')
    
        visualizer = keras.callbacks.TensorBoard(log_dir=os.path.join('datamodify/tensorboard'), 
                                            histogram_freq=0, 
                                            write_graph=True, 
                                            write_images=False)
    ...
    def train_target_discriminator(self, source_model=None, src_discriminator=None, tgt_discriminator=None, epochs=2000, batch_size=100, save_interval=1, start_epoch=0, num_batches=100):   
        ...
        ##load source_model weight from you trained to define_source_encoder
        for layer in self.source_encoder.layers:
            layer.trainable = False
        ## turn trainable to flase 
        ...
        ## if discriminator_weight not none load weight
        source_discriminator.compile(loss = "binary_crossentropy", optimizer=self.src_optimizer, metrics=['accuracy'])########src_optimizer
        target_discriminator.compile(loss = "binary_crossentropy", optimizer=self.src_optimizer, metrics=['accuracy'])
        ##compile two discriminator
        callback1 = keras.callbacks.TensorBoard('datamodify/tensorboard')
        callback1.set_model(source_discriminator)
        callback2 = keras.callbacks.TensorBoard('datamodify/tensorboard')
        ...
        for iteration in range(start_epoch, epochs):
            ...
            for mnist,svhn in zip(src_datagen.flow(source_x, None, batch_size=batch_size), tgt_datagen.flow(target_x, None, batch_size=batch_size)):
                l1, acc1 = source_discriminator.train_on_batch(svhn, np_utils.to_categorical(np.zeros(svhn.shape[0]),num_classes=2))
                l2, acc2 = target_discriminator.train_on_batch(mnist, np_utils.to_categorical(np.ones(mnist.shape[0]),num_classes=2))
                index+=1
                loss, acc = (l1+l2)/2, (acc1+acc2)/2
                #print(target_discriminator.predict_on_batch(mnist))
            ...
            if iteration%save_interval==0:
                source_discriminator.save_weights('datamodify/discriminator_svhn_%02d.hdf5'%iteration)
                target_discriminator.save_weights('datamodify/discriminator_mnist_%02d.hdf5'%iteration)
                target_discriminator.summary()########################
    ...
    def eval_target_classifier(self, source_model, target_discriminator, dataset='mnist'):
        ...
        print('load_weights_from_target_discriminator')
        print('model:')
        model.summary()
        #print('target_discriminator:')
        #target_discriminator.summary()
        #print('source_model:')
...
if __name__ == '__main__':
    ...
    ap.add_argument('-l', '--lr', type=float, default=0.00001, help="Initial Learning Rate")
